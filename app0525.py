...
# 2. 初始化核心底座（API与内置知识库）
# ==========================================
api_key = "sk-d5fb3d6cbb7646f6a1082ef14720c93e"
...
# ==========================================
# 5. 核心交互流（移出 Form，恢复原生 chat_input 以实现平滑滚动与自动清空）
# ==========================================
with st.bottom:
    ...
    with col1:
        # 使用原生 chat_input 保证输入后自动清空，并激活页面的跟随向下滚动
        user_input = st.chat_input("在此处输入补充说明或日常对话，敲回车或点击右侧按钮触发...")
    with col2:
        # 通过普通的 button 代替 form_submit_button，防止死锁
        start_analysis_btn = st.button("🚀 开始分析", use_container_width=True)

    # 聚合触发条件：点击右侧按钮 或 在 chat_input 敲了回车
    start_analysis = start_analysis_btn or user_input

    # 后置拦截空输入
    if start_analysis and (uploaded_file is None and not (user_input and user_input.strip())):
        st.warning("⚠️ 请先上传招标书文件或输入补充说明后再开始分析。")
...
if start_analysis:
    text_hint = user_input.strip() if user_input else ""
        ...
        with placeholder.container():
            with st.spinner(loading_text):
                try:
                    ...
                    # 流式打字机输出：脱离 Form 限制后，这里输出时页面会平滑自动下滑
